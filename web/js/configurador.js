...
(function () {
  ...
  const FABRIC_PROPS = {
    cuero:      { roughness: 0.5,  bumpScale: 0.012 },
    lino:       { roughness: 0.95, bumpScale: 0.02 },
    boucle:     { roughness: 1.0,  bumpScale: 0.035 },
    terciopelo: { roughness: 0.62, bumpScale: 0.008 },
  };
  ...
  const Cfg = {
    state: {
      ...
      madera: 'roble', sala: 'moderno', luz: 'dia', postura: 'sentado',
    },
    targets: { rotY: -0.55, back: 0, foot: 0 },
    current: { rotY: -0.55, back: 0, foot: 0 },
    interacted: false,
    rooms: {},
    ...
    initScene() {
      ...
      this.fabricMat = new THREE.MeshStandardMaterial({ color: 0xDCC8A6, roughness: 0.95 });
      this.woodMat = new THREE.MeshStandardMaterial({
        color: 0xB98E5A, roughness: 0.55, bumpMap: makeBump('madera'), bumpScale: 0.01,
      });
      this.metalMat = new THREE.MeshStandardMaterial({ color: 0x8A7A5C, roughness: 0.4, metalness: 0.7 });
      this.plinthMat = new THREE.MeshStandardMaterial({ color: 0x2A2118, roughness: 0.8 });
        ...
        [-1, 1].forEach(s => {
          ...
          if (cfg.armStyle === 'curved') {
            const roll = new THREE.Mesh(
              new THREE.CylinderGeometry(armT / 2 + 0.012, armT / 2 + 0.012, DEP - 0.06, 14),
              fab
            );
            roll.rotation.x = Math.PI / 2;
            roll.position.set(x, baseY + 0.15 + armH, 0);
      ...
      if (cfg.legStyle === 'wood-tapered' || cfg.legStyle === 'wood-straight') {
        ...
        [[-1, -1], [1, -1], [-1, 1], [1, 1]].forEach(([sx, sz]) => {
          const leg = new THREE.Mesh(
            new THREE.CylinderGeometry(taper ? 0.024 : 0.022, taper ? 0.013 : 0.02, lh, 10), wood
          );
        ...
        add(new THREE.CylinderGeometry(0.035, 0.045, 0.16, 14), this.metalMat, 0, 0.12, 0);
        add(new THREE.CylinderGeometry(0.3, 0.32, 0.025, 28), this.metalMat, 0, 0.015, 0);
      } else { // hidden — zócalo
        add(roundedBoxGeo(W * 0.92, 0.08, DEP * 0.86, 0.04), this.plinthMat, 0, 0.05, 0);
      }
    ...
    animate() {
      ...
      // giro suave de cortesía hasta que el usuario toca
      if (!this.interacted) this.targets.rotY += 0.0022;
